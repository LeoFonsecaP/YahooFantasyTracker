function Season2(){
    return(
        <div className="S1">
            <h1>Ninguém torce pra contender - 2019/2020 Season*</h1>
            <div className="Season1">
                <table>
                    <tr>
                        <th>Rank</th>
                        <th>Team</th>
                        <th>W-L</th>
                        <th>Pct (%)</th>
                        <th>Points</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td className="name">CAMPEÃO TEMP 19/20</td>
                        <td>17-2</td>
                        <td>89.5</td>
                        <td>25379.80</td>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td className="name">Rockit</td>
                        <td>14-5</td>
                        <td>73.7</td>
                        <td>23935.50</td>
                    </tr>
                    <tr>
                        <td>3</td>
                        <td className="name">RIP KOBE</td>
                        <td>14-5</td>
                        <td>73.7</td>
                        <td>23394.50</td>
                    </tr>
                    <tr>
                        <td>4</td>
                        <td className="name">U LOST TO JOE INGLES</td>
                        <td>13-6</td>
                        <td>68.4</td>
                        <td>23281.20</td>
                    </tr>
                    <tr>
                        <td>5</td>
                        <td className="name">SP Jaguars</td>
                        <td>12-7</td>
                        <td>63.2</td>
                        <td>23705.50</td>
                    </tr>
                    <tr>
                        <td>6</td>
                        <td className="name">New York TitaKnicks</td>
                        <td>10-9</td>
                        <td>52.6</td>
                        <td>23439.60</td>
                    </tr>
                    <tr>
                        <td>7</td>
                        <td className="name">Atanktla</td>
                        <td>9-10</td>
                        <td>47.4</td>
                        <td>20906.50</td>
                    </tr>
                    <tr>
                        <td>8</td>
                        <td className="name">Sexland</td>
                        <td>8-11</td>
                        <td>42.1</td>
                        <td>21951.10</td>
                    </tr>
                    <tr>
                        <td>9</td>
                        <td className="name">LuKp Dynamic Duo</td>
                        <td>8-11</td>
                        <td>42.1</td>
                        <td>19193.10</td>
                    </tr>
                    <tr>
                        <td>10</td>
                        <td className="name">Minas Storm</td>
                        <td>4-15</td>
                        <td>21.1</td>
                        <td>16801.70</td>
                    </tr>
                    <tr>
                        <td>11</td>
                        <td className="name">Seattle Super Tank</td>
                        <td>4-15</td>
                        <td>21.1</td>
                        <td>15698.10</td>
                    </tr>
                    <tr>
                        <td>12</td>
                        <td className="name">San Andread Brick</td>
                        <td>1-18</td>
                        <td>0.53</td>
                        <td>11688.80</td>
                    </tr>
                </table>
                <div className = "Categorias">
                    <h2>Champion*: CAMPEÃO TEMP 19/20 (17-2)</h2>
                    <h2>Last Place: San Andreas Brick (1-18)</h2>
                    <h2>MVP: James Harden (3831.30 points)</h2>
                    <h2>ROTY: Ja Morant (2195.30 points)</h2>
                </div>
            </div>
        </div>
        
    );
}

export default Season2;